...
const MakeAttackCard = () => {
    ...
    function resetAttackCard() {
        const attackBonusElement = document.getElementById('attackBonus');
        attackBonusElement.value = 0;
        
        const damageTypeElement = document.getElementById('typeOfDamage');
        damageTypeElement.value = '---';

        const diceTypeElement = document.getElementById('typeOfHitDice');
        diceTypeElement.value = '---';
        
        setRangedOrMeleeAnswer('');
        setNewAttackName('');
        setNewAttackNotes('');
        setShortRange(5);
        setLongRange(20);
        setNumberOfHitDice(1);
        setFinesse(false);
    }

    return (
        <div>
            <div className="resetAttackCardBox">
                <button 
                    className="attackResetButton"
                    onClick={() => { resetAttackCard() }}>
                    reset
                </button>
            </div>
